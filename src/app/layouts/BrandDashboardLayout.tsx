...
import comifyLogoDark from 'figma:asset/6089597c987a8792ad21b7e936f8b0e1cc14deb1.png';
import comifyLogoLight from 'figma:asset/3a417c5da17316907914535aa2d9da54d5f1bf99.png';
...
export default function BrandDashboardLayout() {
  ...
  const [darkMode, setDarkMode] = useState(() => {
    ...
    const saved = localStorage.getItem('brand_dashboard_dark_mode');
    return saved === 'true';
  });
  ...
  useEffect(() => {
    localStorage.setItem('brand_dashboard_dark_mode', darkMode.toString());
  }, [darkMode]);

  const navigation = [
    { name: 'Overview', href: '/brand/dashboard', icon: LayoutDashboard },
    { name: 'Garments', href: '/brand/garments', icon: Shirt },
    { name: 'Try-On Jobs', href: '/brand/jobs', icon: Zap },
    { name: 'API Keys', href: '/brand/api-keys', icon: Key },
    { name: 'Team Members', href: '/brand/team', icon: Users },
    { name: 'Analytics', href: '/brand/analytics', icon: BarChart3 },
    { name: 'Subscription', href: '/brand/subscription', icon: CreditCard },
      ...
      <AuthProvider>
        <div className={`min-h-screen ${darkMode ? 'bg-black text-white' : 'bg-white text-black'}`}>
          ...
          <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
            ...
            <div className={`fixed inset-y-0 left-0 w-72 ${darkMode ? 'bg-[#1a1a1a]' : 'bg-white'} border-r ${darkMode ? 'border-[#333]' : 'border-[#eeeeee]'}`}>
              <div className="flex items-center justify-between p-6 border-b border-[#eeeeee]">
                <img src={darkMode ? comifyLogoLight : comifyLogoDark} alt="Comify AI" className="h-7" />
          ...
          <div className={`hidden lg:fixed lg:inset-y-0 lg:flex lg:w-72 lg:flex-col ${darkMode ? 'bg-[#1a1a1a]' : 'bg-white'} border-r ${darkMode ? 'border-[#333]' : 'border-[#eeeeee]'}`}>
            <div className="flex items-center justify-between p-6 border-b border-[#eeeeee]">
              <img src={darkMode ? comifyLogoLight : comifyLogoDark} alt="Comify AI" className="h-8" />
            </div>
